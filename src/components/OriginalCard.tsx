import Link from "next/link";

import { PedalImage } from "./PedalImage";
import { formatPrice } from "@/lib/format";
import type { DirectoryResult } from "@/lib/filter";

export function OriginalCard({
  result,
  priority = false,
}: {
  result: DirectoryResult;
  priority?: boolean;
}) {
  const { cheapest, bestSaving, alternatives } = result;

  return (
    <Link
      href={`/pedal/${result.slug}`}
      className="tz-chamfer group relative flex flex-col overflow-hidden bg-white shadow-sm ring-1 ring-stone-200/70 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:ring-amber-400/70 focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none"
    >
      <span className="absolute inset-x-0 top-0 z-10 h-1 origin-left scale-x-0 bg-linear-to-r from-amber-400 via-orange-500 to-rose-500 transition-transform duration-300 group-hover:scale-x-100" />

      <div className="relative aspect-4/3 overflow-hidden bg-white">
        <div className="h-full w-full transition-transform duration-500 group-hover:scale-108">
          <PedalImage
            src={result.imageUrl}
            name={result.name}
            brand={result.brand}
            priority={priority}
          />
        </div>

        <span className="tz-eyebrow absolute top-3 left-3 bg-white/90 px-2.5 py-1 text-stone-700 shadow-sm backdrop-blur-sm">
          {result.brand}
        </span>

        {bestSaving && bestSaving.percent > 0 && (
          <span className="absolute right-0 bottom-0 bg-linear-to-r from-emerald-600 to-emerald-500 px-3 py-1.5 text-sm font-bold text-white">
            −{bestSaving.percent}%
          </span>
        )}
      </div>

      <div className="flex flex-1 flex-col gap-3 p-5">
        <div>
          <h3 className="tz-heading text-base text-stone-900 transition-colors group-hover:text-amber-700">
            {result.name}
          </h3>
          <p className="tz-body mt-1.5 line-clamp-2 text-sm text-stone-500">
            {result.blurb}
          </p>
        </div>

        <div className="mt-auto space-y-2 border-t border-stone-100 pt-3">
          <div className="flex items-baseline justify-between gap-2">
            <span className="text-[11px] font-medium text-stone-400">Original</span>
            <span className="text-sm font-bold text-stone-400 line-through">
              {formatPrice(result.priceGBP)}
            </span>
          </div>

          {cheapest && (
            <div className="flex items-baseline justify-between gap-2">
              <span className="text-[11px] font-medium text-stone-500">
                {alternatives.length}{" "}
                {alternatives.length === 1 ? "alternative" : "alternatives"} from
              </span>
              <span className="tz-heading text-xl text-stone-900">
                {formatPrice(cheapest.priceGBP)}
              </span>
            </div>
          )}
        </div>
      </div>
    </Link>
  );
}
